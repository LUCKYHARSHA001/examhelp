...
// Background worker process for PDF summarization.
// Listens for enqueued jobs via BullMQ, builds the custom Gemini prompt, calls the Gemini service,
// streams generated chunks to a Redis pub/sub channel for live SSE delivery, and saves the final summary to the database.
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      await Summary.findByIdAndUpdate(summaryId, { summaryText: fullText, status: 'DONE' });
      ...
      await Summary.findByIdAndUpdate(summaryId, { status: 'FAILED', errorMsg: userFacingError });
